...
## Pastas auxiliares na raiz
- `deploy/` — `deploy.bat` (fluxo Git → GitHub). Na raiz existe `deploy.bat` que só chama `deploy\deploy.bat`.
- Páginas `.html`, `api*.php`, `config.php` e `manifest.json` permanecem na raiz por URLs e `fetch()` relativos estáveis.
